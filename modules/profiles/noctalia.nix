# Noctalia/Umbriel desktop profile.
#
...
#   * Host modules own hardware facts: keyboard device ids, display names,
#     modes and scales.
#
...
in
{
  ...
  # Host modules add concrete keyboard ids for Caps Lock as Hyper. Keep keyd
  # enabled here so each host only supplies the small hardware adapter.
  ...
  # Umbriel's own config lives here so every host that opts into this profile
  # gets the same WM behavior and shortcut map.
  home-manager.users.todor = {
    ...
    programs.noctalia = {
      ...
      settings = {
        ...
        # them DISABLED by default. The shell tracks idle through
        # ext_idle_notifier_v1, which Umbriel implements, but needs enabled
        # behaviors to do anything.
        #
    ...
    programs.umbriel = {
      ...
      settings = {
        ...
        #
        # Role mapping for this profile (verified on blackbox 2026-09-18):
        ...
        keybinds = {
          ...
          "Ctrl+Alt+Shift+Super+P" = "spawn:${pkgs.xdg-utils}/bin/xdg-open https://github.com/pulls";
