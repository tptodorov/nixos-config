...
# Ported verbatim from Omarchy Quattro's default/bash/fns/ssh-reconnect
# (omacom/omarchy, quattro branch) -- it's what stops a dropped remote
# herdr/tmux/editor session from leaving mouse tracking and the alternate
# screen armed on the local terminal.
#
...
_ssh_interactive() {
  ...
  # Named ssh_argv, not argv: zsh treats `argv` as a live alias for the
  # positional parameters, so `local argv=("$@")` would still be mutated by
  # the `shift` calls below instead of holding a snapshot.
  local -a ssh_argv=("$@")
  ...
  local resolved
  resolved=$(command ssh -G "${ssh_argv[@]}" 2>/dev/null) || return 1
