# Herdr dev-layout helpers, ported verbatim from Omarchy Quattro's
# default/bash/fns/herdr (omacom/omarchy, quattro branch) so `hdl`/`hds`/
# `hdlm`/`hsl` behave identically to the upstream shell functions. They
# mirror tmux's own tdl/tds/tdlm/tsl one-for-one, just driven through
# `herdr pane`/`tab`/`workspace` instead of `tmux split-window`/`send-keys`.
...
hsl() {
  ...
  # Upstream indexes $columns with a 0-based C-style loop; zsh arrays are
  # 1-based by default, which would fetch the wrong pane on every column but
  # the first. Scope ksh_arrays to this function only.
